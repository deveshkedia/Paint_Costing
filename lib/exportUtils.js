...
export function exportToCSV(data, filename) {
  try {
    ...
    // Create download using multiple strategies for maximum compatibility
    ...
    alert(`Successfully exported ${filename}.csv`);
  } catch (err) {
...
export function exportToJSON(data, filename) {
  try {
    ...
    alert(`Successfully exported ${filename}.json`);
  } catch (err) {
...
export function exportCostingToPDF(formulation, filename = 'costing') {
  try {
    ...
    alert(`Error exporting costing PDF: ${err.message}`);
    console.error('Costing PDF export error:', err);
  }
}
