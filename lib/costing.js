...
async function computeFormulationCost(formulationId) {
  ...
  // Fallback to total volume if batch size not explicitly set
  const effectiveBatchSizeLitres = batchSizeLitres || (totalVolumeLitre > 0 ? totalVolumeLitre : null);
  ...
  if (packingIds.length > 0 && effectiveBatchSizeLitres && effectiveBatchSizeLitres > 0) {
    ...
    if (!isMultiPack) {
      ...
      for (const p of packResult.rows) {
        if (p.id === formulation.base_packing_id) {
          ...
          const containersNeeded = Math.ceil(effectiveBatchSizeLitres / containerSize);
      ...
      if (volSum > 0) {
        ...
        const baseVolume = (volBase / volSum) * effectiveBatchSizeLitres;
        const hardenerVolume = (volHard / volSum) * effectiveBatchSizeLitres;
        const componentCVolume = isThreePack ? (volC / volSum) * effectiveBatchSizeLitres : 0;
  ...
  // If no weight but packing cost exists, we can't calculate per-kg
  // This happens when formulation lines have percentages but no quantities
  const packingCostPerKg = (totalWeightKg && totalWeightKg > 0)
    ? packingCostPerBatch / totalWeightKg
    : (packingCostPerBatch > 0 ? packingCostPerBatch : 0);
