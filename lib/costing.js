...
async function computeFormulationCost(formulationId) {
  ...
  // batchSizeKg anchors percent <-> kg conversion. If not explicitly set,
  // fall back to the recipe's own total weight (sum of all qty_kg) so older
  // formulations created before batch size existed still compute a correct
  // cost-per-kg, equivalent to the previous (qty*price, divided by total
  // weight) model — not an inflated per-batch total.
  const explicitBatchSizeKg = formulation.batch_size_kg ? Number(formulation.batch_size_kg) : null;
  const impliedBatchSizeKg = lines.reduce((sum, l) => sum + Number(l.qty_kg || 0), 0);
  const batchSizeKg = explicitBatchSizeKg || (impliedBatchSizeKg > 0 ? impliedBatchSizeKg : null);
  ...
  // Packing cost is per-litre: total = cost_per_litre × volume_in_litres
  ...
  if (packingIds.length > 0) {
    ...
    if (!isMultiPack && batchSizeKg && litreDensityKgPerL && litreDensityKgPerL > 0) {
      // Single-pack: cost_per_litre = cost / container_size, then multiply by volume
      const volumeLitres = batchSizeKg / litreDensityKgPerL;
      for (const p of packResult.rows) {
        if (p.id === formulation.base_packing_id) {
          ...
          packingCostPerBatch += costPerLitre * volumeLitres;
        }
      }
    } else if (isMultiPack && batchSizeKg && blendedDensity > 0 && volumeShares) {
      // Multi-pack: calculate volume for each side based on mix ratio
      const volBase = Number(formulation.mix_ratio_vol_base) || 0;
      const volHard = Number(formulation.mix_ratio_vol_hard) || 0;
      const volC = isThreePack ? Number(formulation.mix_ratio_vol_c) || 0 : 0;
      const volSum = volBase + volHard + volC;
      ...
      if (volSum > 0) {
        ...
        const totalVolumeLitres = batchSizeKg / blendedDensity;
        ...
        for (const p of packResult.rows) {
          ...
          if (p.id === formulation.base_packing_id) {
            packingCostPerBatch += costPerLitre * baseVolume;
          } else if (p.id === formulation.hardener_packing_id) {
            packingCostPerBatch += costPerLitre * hardenerVolume;
          } else if (p.id === formulation.component_c_packing_id && isThreePack) {
            packingCostPerBatch += costPerLitre * componentCVolume;
          }
        }
      }
    }
  ...
  const packingCostPerKg = batchSizeKg && batchSizeKg > 0 ? packingCostPerBatch / batchSizeKg : 0;
